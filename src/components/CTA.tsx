    ...
    <section className="relative bg-gradient-to-r from-primary-600 to-primary-700 py-20 overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12 animate-fade-in-delay">
          <Link
            href="/contact"
            className="bg-white text-primary-600 px-8 py-4 rounded-lg text-lg font-semibold hover:bg-gray-100 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl"
          >
            {CTA_CONTENT.primaryButton}
          </Link>
          <Link
            href="/experts"
            className="border-2 border-white text-white px-8 py-4 rounded-lg text-lg font-semibold hover:bg-white hover:text-primary-600 transform hover:scale-105 transition-all duration-200"
          >
            {CTA_CONTENT.secondaryButton}
          </Link>
        </div>
        
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 animate-fade-in-delay-2">
          {CTA_CONTENT.benefits.map((benefit, index) => (
            <div key={index} className="text-center">
              ...
              <p className="text-sm text-primary-100 font-medium">{benefit}</p>
            </div>
          ))}
        </div>
      </div>
